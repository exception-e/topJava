...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        jdbcTemplate.query("SELECT * FROM user_role", new RowCallbackHandler() {
            @Override
    ...
    private User setRoles(User u){
        if(u!= null){
            List<Role> roles = jdbcTemplate.queryForList("SELECT role FROM user_role WHERE user_id=?",Role.class, u.id());
            u.setRoles(roles);
        }
        return u ;
    }
